...
def login_user(main_root):
    global current_user_id  # Use the global variable

    def submit_login():
        global current_user_id  # Declare the use of the global variable
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if user_data and bcrypt.checkpw(password.encode('utf-8'), user_data[1].encode('utf-8')):
                    current_user_id = user_data[0]  # Store the user ID globally
                    messagebox.showinfo("Success", "Login successful.")
                    login_root.destroy()  # Close the login window
                    user_menu(current_user_id)  # Open user menu with the user ID
                else:
                    messagebox.showwarning("Failed", "Login failed. Please check your credentials.")
        except mysql.connector.Error as err:
            messagebox.showwarning("Database error", str(err))

    login_root = tk.Toplevel(main_root)  # Create a new window for login
    login_root.title("Login User")
    login_root.maxsize(500, 500)

    canvas = tk.Canvas(login_root, bg="black", height=500, width=500)
    canvas.pack()

    username_entry = tk.StringVar()
    password_entry = tk.StringVar()

    welcome = tk.Label(login_root, text="Login", bg="yellow", relief="raised", width=30, height=3, font=("Segoe UI", 12))
    welcome.place(x=140, y=30)

    uname1 = tk.Label(login_root, text="Username", relief="raised", width=20, height=2, font=("Segoe UI", 8))
    uname1.place(x=200, y=140)

    uname_fill1 = tk.Entry(login_root, textvariable=username_entry, relief="sunken")
    uname_fill1.place(x=200, y=185)

    password1 = tk.Label(login_root, text="Password", relief="raised", width=20, height=2, font=("Segoe UI", 8))
    password1.place(x=200, y=235)

    password_fill1 = tk.Entry(login_root, textvariable=password_entry, show='*', relief="sunken")
    password_fill1.place(x=200, y=280)

    submit_button = tk.Button(login_root, text="Submit", relief="raised", bg="cyan", width=20, height=2, font=("Segoe UI", 8),
...
def register_offline_product(user_id):
    ...
    warranty = tk.Label(root, text = "Warranty period:", bg = "white", relief="raised",
                   width = 30, height = 3, font = ("Segoe UI", 8))
    warranty_entry = tk.Entry(root, relief="sunken", font = ("Segoe UI", 8))
    ...
    warranty_code = tk.Label(root, text = "Warranty code:", bg = "white", relief="raised",
                   width = 30, height = 3, font = ("Segoe UI", 8))
    warranty_code_e = tk.Entry(root, relief="sunken", font = ("Segoe UI", 8))
    ...
    purchase_bill_path = tk.Label(root, text = "Purchase Bill Path:", bg = "white", relief="raised",
                   width = 30, height = 3, font = ("Segoe UI", 8))
    purchase_bill_entry = tk.Entry(root, relief="sunken", font = ("Segoe UI", 8))
    purchase_bill_path.place(x = 140, y = 390)
    purchase_bill_entry.place(x = 350, y = 394)

    def offlineReg(product_name, seller, purchase_date, warranty_period, warranty_code, purchase_bill_path):
        ...
        """
        product_data = (
        user_id, product_name, seller, purchase_date, warranty_period, warranty_code, purchase_bill_path)

        try:
            with conn.cursor() as cursor:
                cursor.execute(insert_product_query, product_data)
                conn.commit()
                messagebox.showinfo("Successful!", "Product registration successful.")
        except mysql.connector.Error as err:
            messagebox.showwarning("Error", f"Database error: {err}")

    submit_button = tk.Button(root, text="Submit", bg="yellow", relief="raised", width=30, height=2,
                              font=("Segoe UI", 9),
                              command=lambda: offlineReg(prodname.get(), sellername.get(), pdatename.get(),
                                                         warrantyname.get(), warranty_code.get(), purchasebill.get()))
...
def register_online_product(user_id):
    ...
    def online_prod( product_name, purchase_date, warranty_period, warranty_code, source_url):
        insert_product_query = """
        ...
        try:
            ...
            messagebox.showwarning("Warning", "Try again later")

    #online_prod = partial(online_prod, product, sellern, pdateval, warrantyval, war_code, source_url_entry1)

    submit_button = tk.Button(root, text="Submit", bg="yellow", relief="raised", width=30, height=2, font=("Segoe UI", 9),
                              command=lambda: online_prod(product.get(), sellern.get(), pdateval.get(), warrantyval.get(), war_code.get(), source_url_entry1.get()))
    submit_button.place(x=140, y=450)
...
def main():
    ...
    login = tk.Button(root, text="Login", relief="raised", width=20, height=2, font=("Segoe UI", 8), command=lambda: login_user(root))
    login.place(x=200, y=140)
    login.place(x = 200, y = 140)
    
    register = tk.Button(root, text="Register", relief="raised", width=20, height=2, font=("Segoe UI", 8), command=lambda: register_user(root))
...
def user_menu(user_id):
    ...
    online_reg = tk.Button(root, text = "Register products online", relief="raised",
                       width = 20, height = 3, font = ("Segoe UI", 9), command = register_online_product(user_id))
    online_reg.place(x = 200, y = 150)

    #offline reg
    offline_reg = tk.Button(root, text = "Register products offline", relief="raised",
                       width = 20, height = 3, font = ("Segoe UI", 9), command = register_offline_product(user_id))
